Shader "Unlit/grass"
{
	Properties
	{
		...
		_StandardMaxHeight("Standard Blade Height Max", Range(0, 2)) = 0.2
		_StandardMinHeight("Stamdard Blade Height Min", Range(0, 2)) = 0.1
	...
	SubShader
	{
		...
		HLSLINCLUDE
			...
			CBUFFER_START(UnityPerMaterial)
				...
				float _StandardMaxHeight;
				float _StandardMinHeight;
			...
			void geom(point VertexOutput input[1], inout TriangleStream<GeomData> triStream)
			{
				...
				{
					height = lerp(_StandardMinHeight, _StandardMaxHeight, rand(pos.zyx) * falloff);
				}
				else
				{
					height = lerp(_BladeHeightMin, _BladeHeightMax, rand(pos.zyx) * falloff);
				}
